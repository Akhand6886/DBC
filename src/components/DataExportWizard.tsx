...
export const DataExportWizard: React.FC<DataExportWizardProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 font-mono text-xs">
      <div className="bg-ide-sidebar border border-ide-border rounded-2xl max-w-md w-full p-5 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
        {/* Header */}
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            <label className="text-[10px] text-slate-400 font-bold uppercase">Export File Format:</label>
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: 'excel', name: 'Excel (.xlsx)', icon: <FileSpreadsheet className="h-4 w-4 text-emerald-400" /> },
                { id: 'csv', name: 'CSV (.csv)', icon: <FileText className="h-4 w-4 text-cyan-400" /> },
                { id: 'json', name: 'JSON (.json)', icon: <FileJson className="h-4 w-4 text-yellow-400" /> },
                { id: 'markdown', name: 'Markdown', icon: <Code className="h-4 w-4 text-purple-400" /> },
                { id: 'html', name: 'HTML Table', icon: <Database className="h-4 w-4 text-sky-400" /> },
              ].map((item) => (
                <button
                  key={item.id}
                  type="button"
                  ...
                  className={`p-2 rounded-lg border flex flex-col items-center justify-center space-y-1.5 transition-all ${
                    format === item.id ? 'border-cyan-500 bg-cyan-500/10 text-white shadow' : 'border-ide-border bg-ide-bg text-slate-400 hover:text-slate-200'
                  }`}
                >
                  {item.icon}
